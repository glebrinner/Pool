...
print("\033[1;36mStarting development build for mobile platforms (0.%d)\033[0m" % BUILD_NUMBER)
...
with open(uproject_file) as json_file:
    ...
    for i in to_delete:
        ...
        deleted += 1
    data["Plugins"].append({"Name": "GooglePAD", "Enabled": False})
...
with open ('Config/DefaultEngine.ini', 'r' ) as f:
    ...
    default_engine = re.sub('bPackageDataInsideApk=.*$', r'bPackageDataInsideApk=True', default_engine, flags = re.M)
    default_engine = re.sub('bBuildForArmV7=.*$', r'bBuildForArmV7=False', default_engine, flags = re.M)
    default_engine = re.sub('bEnableBundle=.*$', r'bEnableBundle=False', default_engine, flags = re.M)
    default_engine = re.sub('MobileProvision=.*$', r'MobileProvision=SefaPoolDevelopmentMobileProvisioning.mobileprovision', default_engine, flags = re.M)
    default_engine = re.sub('SigningCertificate=.*$', r'SigningCertificate=iPhone Developer: Raffaele Della Pietra (SB3NNS828M)', default_engine, flags = re.M)
...
with open ('Config/DefaultGame.ini', 'r' ) as f:
    ...
    default_game = re.sub('ForDistribution=.*$', r'ForDistribution=False', default_game, flags = re.M)
...
with open ('Source/SefaPool/Version/Version.h', 'r' ) as f:
    ...
    version_header = re.sub('build = .*$', r'build = TEXT("Test build\\n%s. \\nBranch %s");'%(dt_string, branch), version_header, flags = re.M)
...
os.system("ue4 uat -ScriptsForProject=%s/%s BuildCookRun -nocompileeditor -installed -nop4 -project=%s/%s -cook -stage -archive -archivedirectory=%s/Packaging -package -compressed -ddc=InstalledDerivedDataBackendGraph -pak -prereqs -nodebuginfo -targetplatform=IOS -build -target=%s -clientconfig=Development -utf8output" % 
